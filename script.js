...
function scoresToDOM() {
  ...
  wrongCountEl.textContent= `Incorrect: ${countWrong}`;
  updateBestScore();
...
function startTimer() {
  ...
  finalTime = 0;
  countWrong = 0;
  timer = setInterval(()=>{timePlayed = timePlayed+0.1 }, 100);
...
function createEquations() {
  ...
  for (let i = 0; i < wrongEquations; i++) {
    ...
    wrongFormat[1] = `${firstNumber} x ${secondNumber} = ${equationValue + getRandomInt(10)}`;
...
function selectQuestionAmount(e) {
  ...
  if (questionAmount) {
    ...
    alert('please choose one option below!!!')
  }
}
